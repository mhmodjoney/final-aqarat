...
exports.update = async (fields) => {
  ...
  const [rows] = await pool.query(
    'CALL sp_crud_user(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
    ["update",fields.user_id,fields.full_name,fields.phone_number,fields.whatsapp_number,fields.email,fields.user_name,fields.password,null,null,null,fields.user_name,null,null,null,null]
  );
...
exports.profileId = async (fields)=> {
  const [rows] = await pool.query(
    'CALL sp_crud_user(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
...
exports.profileUserName = async (fields)=> {
  const [rows] = await pool.query(
    'CALL sp_crud_user(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
    ["user_name",fields.user_id,null,null,null,null,null,null,null,null,null,null,null,null,null,null]
  )
  return rows[0][0];
